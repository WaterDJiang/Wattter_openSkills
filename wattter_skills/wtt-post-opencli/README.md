...
- 不做反检测式"模拟真人"动作，不随机移动鼠标、不随机点击、不伪装打字节奏、不绕过验证码或风控。
- 遇到验证码、异常登录、频控、账号风险提醒、内容审核拦截，停止自动发布并让用户在平台侧处理。
- 批量跨发时低频、逐个平台执行、逐个平台校验；不要因为某个平台失败而重复发同一条内容。
- 优先使用 OpenCLI 已支持的适配器和平台允许的流程，保留账号身份、返回 URL、草稿状态和最新帖子列表等校验证据。
