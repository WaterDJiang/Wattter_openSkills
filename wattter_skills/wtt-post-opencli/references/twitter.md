...
- 不要为 Twitter/X 加入规避检测的模拟真人动作。遇到验证码、登录风险、频率限制、账号异常或发送按钮不可用时停止自动化，交给用户人工处理。
...
- Twitter/X 正文默认按纯文本处理。不要保留 Markdown H1、`##`、`---`、HTML 注释或不必要的 `**` 符号；如果需要标题，把标题改成自然首句，不要用 `# 标题`。
- 图片、视频独立放在 `media`。当前适配器不支持某类媒体时，先让用户选择纯文本、改素材、浏览器待确认或跳过。
- 图片必须是稳定路径。不要引用脚本临时海报或发布后会被清理的文件。
...
浏览器回退只允许用于正常页面操作和人工确认，不允许添加随机鼠标轨迹、随机滚动、随机停顿、模拟打字习惯或绕过验证码。平台出现风险提示时，立即停止。
