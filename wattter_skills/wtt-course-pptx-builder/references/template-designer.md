...
> This is a standalone role: triggered via the `/create-template` workflow (library mode) **or** the `/use-user-template` workflow (project mode). In library mode it is **not** the project-level template selection/customization step in the main PPT generation pipeline.

## Usage

- **Trigger**: `/create-template` workflow (library mode) **or** `/use-user-template` workflow Step 3a (project mode)
- **Output location**:
  - Library mode: `templates/<kind_dir>/<template_name>/` where `<kind_dir>` is `decks` (default — full-PPT replica with identity) or `layouts` (structure-only, no identity segment)
  - Project mode: `<project>/templates/` — one-off use by the current project; **does not** call `register_template.py` (library save is an optional separate step in `use-user-template.md` Step 4)
...
> **Project mode differences from library mode** (everything else unchanged): output lands in `<project>/templates/` instead of the global library; no `register_template.py` call; the `[TEMPLATE_BRIEF_CONFIRMED]` gate is replaced by the depth sub-choice already made in `use-user-template.md` Step 2 (still present a brief — colors/fonts/replication mode/page roster — and get user confirmation before generating). Replication rules (standard/fidelity/mirror), sprite preservation, and the personality-only `design_spec.md` skeleton are identical.
