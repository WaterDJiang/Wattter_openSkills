...
- **Slim lookup (discovery only)**: [layouts/layouts_index.json](./layouts/layouts_index.json) — populates the Step 3 flow 1 menu and answers "what templates exist?". Listing alone does not advance the pipeline; the user picks via flow 1 or gives an explicit path.

## Brand Identity Presets

The `brands/` directory holds brand-only templates: identity bundles (color / typography / logo / voice / icon style) without an SVG page roster. Brands follow the **same three-flow gate as layout templates** (SKILL.md Step 1, flow 1) — the user picks the brand from the flow 1 menu or supplies its directory path; bare brand names never auto-select. Both layout and brand inputs land in the same project directory (`<project_path>/templates/`). When selected together, Step 3 fuses them into a single `design_spec.md` (brand wins on identity tokens, layout wins on page structure) — see `SKILL.md` Step 3 for the precedence table.
...
- **Discovery index**: [brands/brands_index.json](./brands/brands_index.json) — populates the flow 1 menu and answers "what brands exist?"; the user still picks via flow 1 or gives an explicit path.
