...
(function(){
  ...
  window.HPX['counter-explosion'] = function(el){
    ...
    num.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font:900 120px system-ui,sans-serif;color:var(--text-1,#fff);pointer-events:none;text-shadow:0 4px 40px rgba(124,92,255,0.5);z-index:1;';
    num.textContent = '0';
    el.appendChild(num);
    let parts = [];
    let state = 'count';
    ...
    const burst = function() {
      ...
      for (let i = 0; i < 120; i++){
        ...
        parts.push({
          ...
          life:1, r:U.rand(2,5), c:pal[(Math.random()*pal.length)|0]
        });
      }
    };
    const stop = U.loop(function() {
      ...
      if (state === 'count'){
        ...
        if (stateT > 1.5){
          state = 'count'; stateT = 0; value = 0; num.textContent = '0';
        }
      }
