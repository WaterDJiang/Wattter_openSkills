...
生成一份**自包含文件夹**的横向翻页网页演示文稿。当前支持两种组件化风格：
...
1. **确认输入完整度。** 如果用户已经给了完整大纲、素材和图片，直接进入生成；如果只有主题或模糊想法，先问：风格、受众场景、分享时长、原始素材、图片、主题色、动效模式、预设/布局序列、硬约束。
2. **选风格。** 读 `references/styles/index.md`。拿不准时默认 `editorial`；内容偏事实、产品、分析、方法论、数据汇报时优先 `swiss`。
3. **选引用资料。**
...
4. **生成 deck 文件夹。** 输出目录必须命名为 `<YYYYMMDDHHmm>_<内容主题>_Mdeck`，例如 `202606171500_个人业务介绍_Mdeck`；目录内结构必须是可直接打开的静态文件夹：`index.html`、`css/`、`js/`、`images/`。
5. **运行校验。** 生成后必须先跑 `node scripts/validate-theme-contract.mjs`，再跑 `node scripts/validate-deck.mjs <index.html> <style-id>`；Swiss 额外跑 `node scripts/validate-swiss-deck.mjs <index.html>`。
...
```bash
TOPIC="个人业务介绍"   # 用 deck 内容主题，不要用 generic deck / slides / output
STAMP="$(date +%Y%m%d%H%M)"
SAFE_TOPIC="$(printf '%s' "$TOPIC" | tr ' /\\:：' '-----')"
DIR="项目/XXX/${STAMP}_${SAFE_TOPIC}_Mdeck"
...
cp <SKILL_ROOT>/assets/webgl/grid.js "$DIR/js/webgl.js"   # swiss
...
- 输出目录必须使用内容主题命名，并以生成时间开头：`YYYYMMDDHHmm_内容主题_Mdeck`。不要输出到 `deck`、`dist`、`output`、`slides` 这类泛目录。
...
- Swiss 不要用内联 `color:var(--paper)` 或 `color:var(--ink)` 硬改整页文字；深浅底色用 `slide.dark`、`b-ink`、`b-accent` 等登记 class，让 CSS 处理可读色。
